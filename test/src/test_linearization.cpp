...
void get_vo_estimator(int num_frames, basalt::BundleAdjustmentBase<Scalar>& estimator, basalt::AbsOrderMap& aom) {
  static constexpr int POSE_SIZE = 6;

  estimator.huber_thresh = 0.5;
  estimator.obs_std_dev = 2.0;

  estimator.calib.T_i_c.emplace_back(Sophus::se3_expd(Sophus::Vector6d::Random() / 100));
  estimator.calib.T_i_c.emplace_back(Sophus::se3_expd(Sophus::Vector6d::Random() / 100));
  ...
  for (int i = 0; i < num_frames; i++) {
    ...
    for (int j = 0; j < 10; j++) {
      ...
      kpt.direction = basalt::StereographicParam<Scalar>::project(p3d_cam.homogeneous());
      ...
      for (const auto& [frame_id, frame_pose] : estimator.frame_poses) {
        for (int c = 0; c < 2; c++) {
          ...
          Sophus::SE3d T_c_w = estimator.calib.T_i_c[c].inverse() * frame_pose.getPose().inverse();
...
template <class Scalar>
void get_vo_estimator_with_marg(int num_frames, basalt::BundleAdjustmentBase<Scalar>& estimator,
                                basalt::AbsOrderMap& aom, basalt::MargLinData<Scalar>& mld) {
  static constexpr int POSE_SIZE = 6;
...
TEST(LinearizationTestSuite, VoNoMargLinearizationTest) {
  ...
  l_abs_qr = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator, aom, options);
  ...
  l_abs_sc = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator, aom, options);
  ...
  l_rel_sc = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator, aom, options);
...
TEST(LinearizationTestSuite, VoMargLinearizationTest) {
  ...
  l_abs_qr = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator, aom, options, &mld);
  Scalar error_abs_qr = l_abs_qr->linearizeProblem();
  l_abs_qr->performQR();
  ...
  options.linearization_type = basalt::LinearizationType::ABS_SC;
  std::unique_ptr<basalt::LinearizationBase<Scalar, POSE_SIZE>> l_abs_sc;
  l_abs_sc = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator, aom, options, &mld);
  ...
  options.linearization_type = basalt::LinearizationType::REL_SC;
  std::unique_ptr<basalt::LinearizationBase<Scalar, POSE_SIZE>> l_rel_sc;
  l_rel_sc = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator, aom, options, &mld);
...
TEST(LinearizationTestSuite, VoMargBacksubstituteTest) {
  ...
  basalt::BundleAdjustmentBase<Scalar> estimator2 = estimator, estimator3 = estimator;
  ...
  options.linearization_type = basalt::LinearizationType::ABS_SC;
  std::unique_ptr<basalt::LinearizationBase<Scalar, POSE_SIZE>> l_abs_sc;
  l_abs_sc = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator2, aom, options, &mld);
  ...
  options.linearization_type = basalt::LinearizationType::REL_SC;
  std::unique_ptr<basalt::LinearizationBase<Scalar, POSE_SIZE>> l_rel_sc;
  l_rel_sc = basalt::LinearizationBase<Scalar, POSE_SIZE>::create(&estimator3, aom, options, &mld);
...
TEST(LinearizationTestSuite, VoMargSqrtLinearizationTest) {
  ...
  Eigen::MatrixXd H_abs_qr, H_abs_sc, H_rel_sc, Q2TJp_abs_qr, Q2TJp_rel_sc, Q2TJp_abs_sc;
  Eigen::VectorXd b_abs_qr, b_abs_sc, b_rel_sc, Q2Tr_abs_qr, Q2Tr_rel_sc, Q2Tr_abs_sc;
